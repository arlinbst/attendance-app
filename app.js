...
let currentCameraMode = 'environment'; // 'environment' = back camera, 'user' = front camera
...
function toggleCamera() {
    const wasScanning = html5QrcodeScanner !== null;
    
    // Toggle camera mode
    if (currentCameraMode === 'environment') {
        currentCameraMode = 'user';
        document.getElementById('camera-toggle-btn').innerHTML = '📷 Switch to Back Camera';
    } else {
        currentCameraMode = 'environment';
        document.getElementById('camera-toggle-btn').innerHTML = '📷 Switch to Front Camera';
    }
